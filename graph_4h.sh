for filename in ./dbs/*; do
    ...
    rrdtool graph graphs/"${name/.rrd/}_4h".png \
    --imgformat PNG \
    --title="Server latency history (last 4 hours)" \
    ...
    DEF:pingminn=dbs/$name:ping:MIN:step=300 \
    DEF:pingmaxx=dbs/$name:ping:MAX:step=300 \
